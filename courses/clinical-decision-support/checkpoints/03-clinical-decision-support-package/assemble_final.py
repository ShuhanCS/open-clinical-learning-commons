"""Freeze an accepted APP-4 Module 07 candidate for final review."""
...
CP1_RELEASE = COURSE_ROOT / "checkpoints/01-logic-evidence-validation-readiness/release.json"
CP2_RELEASE = COURSE_ROOT / "checkpoints/02-workflow-sandbox-safety-release/release.json"
...
def candidate_inventory(candidate: Path) -> list[dict[str, object]]:
    ...
    if fields != MODULE_MANIFEST_FIELDS or len(immutable) != 1320:
    ...
    if len(paths) != 1347 or len(set(paths)) != 1347:
        raise ValueError("Module 07 candidate must contain exactly 1,347 unique files")
    return [{
...
def assemble(candidate: Path, target: Path, reference: bool = False, hardlink: bool = False) -> dict[str, object]:
    ...
    if actual != expected or len(actual) != 1362:
        raise ValueError("Final checkpoint tree must contain exactly 1,362 expected files")
    return {
...
def self_check() -> None:
    with tempfile.TemporaryDirectory(prefix="app4-final-assemble-") as temp_dir:
        base = Path(temp_dir)
        ...
        assert one == two
        assert one["candidate_files"] == 1347 and one["assembled_files"] == 1362
    ...
    print("APP-4 final assembler self-check passed: 1,347 frozen candidate files and 1,362 total files.")
...
def main() -> None:
    ...
    try:
        if args.reference:
            ...
            with tempfile.TemporaryDirectory(prefix="app4-final-reference-") as temp_dir:
