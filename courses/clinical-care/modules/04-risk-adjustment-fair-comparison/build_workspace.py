"""Assemble APP-1 Module 04 learner or reference workspaces."""
...
OUTPUT_FILES = (
    ...
    "calibration-quintiles.csv", "comparison-figure.svg", "expected-outcomes.csv", "exposure-comparison.csv",
    "field-role-summary.csv", "model-coefficients.csv", "model-performance.csv", "site-case-mix.csv",
    "site-comparison.csv",
)
...
def assemble(target: Path, reference: bool = False) -> dict[str, object]:
    ...
    for relative in IMMUTABLE_FILES:
        ...
        manifest.append({
            ...
            "role": "immutable analysis environment assessment field-role or executable control",
        })
    ...
    expected_files = 32 if reference else 19
    actual_files = sum(path.is_file() for path in target.rglob("*"))
    if len(manifest) != 10 or actual_files != expected_files:
    ...
    return {
        ...
        "assembled_files": actual_files,
    }


def self_check() -> None:
    with tempfile.TemporaryDirectory(prefix="app1-module04-workspace-") as temp_dir:
        base = Path(temp_dir)
        ...
        assert first["assembled_files"] == 32 and starter["assembled_files"] == 19
        assert "REPLACE" not in (one / "risk-adjustment-memo.md").read_text(encoding="utf-8")
        assert "REPLACE" in (learner / "risk-adjustment-memo.md").read_text(encoding="utf-8")
        try:
    ...
    print("APP-1 Module 04 workspace builder self-check passed: 10 immutable rows and 19/32 learner/reference files.")
