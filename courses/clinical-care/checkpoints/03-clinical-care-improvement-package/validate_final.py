"""Validate APP-1 final clinical care improvement packages."""
...
MODULE_MANIFEST_BYTES = 40140
MODULE_MANIFEST_SHA256 = "2c90713fb220b6fdc1af492898e89605051b0dffed44b2fb2883b2942aefde62"
MODULE_RELEASE_SHA256 = "939fc4c08f46974e3d7db1c2b387f2c7251287cdba2317e2867ce326edbac933"
CP1_RELEASE_SHA256 = "ef2ee1dd1fcac47dda2efd680b9605862a0006962867d59a836dec4c276b090c"
CP2_RELEASE_SHA256 = "58cc270fad6649feec5e958b0850ea3dff3a8119599c30f2e900d68ad5f591da"
...
def validate(root: Path, starter: bool = False) -> dict[str, object]:
    ...
    require(len(final_manifest) == 236, "Final candidate manifest has 236 rows")
    candidate_paths = [row["relative_path"] for row in final_manifest]
    require(candidate_paths == sorted(candidate_paths) and len(set(candidate_paths)) == 236, "Final candidate paths are sorted and unique")
    ...
    require(actual == set(candidate_paths) | FINAL_FILES and len(actual) == 251, "Final package has exactly 251 expected files")

    module_header, module_manifest = read_csv(root / "release-manifest.csv")
    require(module_header == MODULE_MANIFEST_FIELDS and len(module_manifest) == 214, "Module 07 immutable manifest has 214 rows")
    ...
    require(sha256(root / "final-review/checkpoint1-release.json") == CP1_RELEASE_SHA256, "Checkpoint 1 release identity matches")
    require(sha256(root / "final-review/checkpoint2-release.json") == CP2_RELEASE_SHA256, "Checkpoint 2 release identity matches")
    require(sha256(root / "final-review/module07-release.json") == MODULE_RELEASE_SHA256, "Module 07 release identity matches")
    require(sha256(root / "evidence/provenance/checkpoint1-release.json") == CP1_RELEASE_SHA256, "Nested Checkpoint 1 release identity matches")
    require(sha256(root / "evidence/provenance/checkpoint2-release.json") == CP2_RELEASE_SHA256, "Nested Checkpoint 2 release identity matches")

    with tempfile.TemporaryDirectory(prefix="app1-final-candidate-validate-") as temp_dir:
    ...
    if starter:
        report = {"status": "pass", "mode": "starter", "checks_passed": len(checks), "candidate_files": 236, "assembled_files": 251}
        print(f"APP-1 final starter validation passed: {len(checks)} checks.")
        return report
    ...
    submission_values = (
        "https://github.com/ShuhanCS/open-clinical-learning-commons", "`236`", "`214`", "`40140`",
        MODULE_MANIFEST_SHA256, CP1_RELEASE_SHA256, CP2_RELEASE_SHA256, MODULE_RELEASE_SHA256,
        "official last day of the assigned MGH Institute half-term", "app1-clinical-care-candidate-v0.1.0",
        "proposed - not created", "https://www.mghihp.edu/sites/default/files/2026-06/ihp-calendar-2026-2027-with-winter-term-current.pdf",
    )
    ...
    require(len(conditions) == 6 and condition_ids == {f"COND-{i:02d}" for i in range(1, 7)}, "Six final conditions are registered")
    ...
    require(len(gates) == 24 and [row["gate_id"] for row in gates] == [f"G{i:02d}" for i in range(1, 25)], "Twenty-four final gates are ordered")
    ...
    require(len(re.findall(r"(?m)^## Q(?:0[1-9]|1[0-2])\.", defense)) == 12, "All 12 final defense answers are present")
    require(all(len(section.strip()) >= 90 for section in re.split(r"(?m)^## Q(?:0[1-9]|1[0-2])\..*$", defense)[1:]), "Every final defense answer is substantive")
    ...
    require(all(role in reviewer for role in reviewer_roles), "All ten final reviewer roles are present")
    ...
    require(all(value in reproduction for value in ("`236`", "`251`", "`214`", "Module 07 validator: `pass`", "Two-build match: `pass`")), "Final reproduction package results match")

    audit = (review_root / "final-audit.md").read_text(encoding="utf-8")
    audit_values = ("236 rows", "214 rows", "16 of 16", "none found", "separate and consistent", "prohibited", "pass with conditions for curriculum construction")
    require(all(value in audit for value in audit_values), "Final audit covers identity integrity access accountability and decisions")

    decision = (review_root / "final-decision.md").read_text(encoding="utf-8")
    disposition = markdown_field(decision, "Package disposition")
    recommendation = markdown_field(decision, "Clinical recommendation")
    ...
    require(markdown_field(module_progression, "Clinical recommendation") == recommendation, "Final recommendation matches Module 07")
    ...
    acceptance_values = (
        "Curriculum construction", "Clinical implementation", "model deployment", "patient targeting",
    ...
    report = {
        ...
        "candidate_files": 236, "assembled_files": 251, "manifest_rows": 236,
        "score": total, "gates_passed": 24, "package_disposition": disposition,
        "clinical_recommendation": recommendation,
    }
    print(f"APP-1 final complete validation passed: {len(checks)} checks.")
    return report
...
def self_check() -> None:
    with tempfile.TemporaryDirectory(prefix="app1-final-validate-") as temp_dir:
        base = Path(temp_dir)
        reference = base / "reference"
        command([sys.executable, str(ASSEMBLER), "--target", str(reference), "--reference"])
        report = validate(reference)
        assert report["candidate_files"] == 236 and report["assembled_files"] == 251
        ...
        evidence = changed / "improvement-recommendation.md"
        ...
        expect_failure(changed)
    print("APP-1 final validator self-check passed: complete, starter, candidate, score, gate, tag, and decision routes.")
