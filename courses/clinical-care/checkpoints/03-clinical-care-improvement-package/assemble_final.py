"""Freeze an accepted APP-1 Module 07 candidate for final review."""
...
CP1_RELEASE = COURSE_ROOT / "checkpoints/01-longitudinal-survival-readiness/release.json"
CP2_RELEASE = COURSE_ROOT / "checkpoints/02-adjusted-variation-improvement-release/release.json"
...
def candidate_inventory(candidate: Path) -> list[dict[str, object]]:
    ...
    if fields != MODULE_MANIFEST_FIELDS or len(immutable) != 214:
        raise ValueError("Module 07 immutable manifest changed")
    if (candidate / "release-manifest.csv").stat().st_size != MODULE_MANIFEST_BYTES or sha256(candidate / "release-manifest.csv") != MODULE_MANIFEST_SHA256:
    ...
    if len(paths) != 236 or len(set(paths)) != 236:
        raise ValueError("Module 07 candidate must contain exactly 236 unique files")
    return [{
        "relative_path": relative,
        "bytes": (candidate / safe_relative(relative)).stat().st_size,
        "sha256": sha256(candidate / safe_relative(relative)),
        "role": roles.get(relative, "Module 07 leadership record"),
...
def assemble(candidate: Path, target: Path, reference: bool = False) -> dict[str, object]:
    ...
    release_checks = (
        ...
        (CP1_RELEASE, CP1_RELEASE_SHA256, "Checkpoint 1"),
        (CP2_RELEASE, CP2_RELEASE_SHA256, "Checkpoint 2"),
    )
    ...
    if actual != expected or len(actual) != 251:
        raise ValueError("Final checkpoint tree must contain exactly 251 expected files")
    return {
...
def self_check() -> None:
    with tempfile.TemporaryDirectory(prefix="app1-final-assemble-") as temp_dir:
        base = Path(temp_dir)
        ...
        assert one == two
        assert one["candidate_files"] == 236 and one["assembled_files"] == 251
    ...
    print("APP-1 final assembler self-check passed: 236 frozen candidate files and 251 total files.")
...
def main() -> None:
    ...
    try:
        if args.reference:
            ...
            with tempfile.TemporaryDirectory(prefix="app1-final-reference-") as temp_dir:
