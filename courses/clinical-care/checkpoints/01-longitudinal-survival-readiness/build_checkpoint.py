"""Assemble the APP-1 cumulative Week 3 checkpoint."""
...
MODULES = (
    ("oclc-app1-01", "0.2.0", "module-01", COURSE_ROOT / "modules/01-care-pathway-decision"),
    ("oclc-app1-02", "0.1.0", "module-02", COURSE_ROOT / "modules/02-longitudinal-cohorts-followup"),
    ("oclc-app1-03", "0.1.0", "module-03", COURSE_ROOT / "modules/03-survival-time-to-event"),
)
...
def assemble(target: Path, reference: bool = False) -> dict[str, object]:
    ...
    manifest = []
    with tempfile.TemporaryDirectory(prefix="app1-cp01-modules-") as temp_dir:
        temp = Path(temp_dir)
        for module_id, version, directory, module_root in MODULES:
            workspace = temp / directory
            report = load_builder(module_id, module_root).assemble(workspace, reference=True)
            expected = {"module-01": 19, "module-02": 30, "module-03": 29}[directory]
            if report["assembled_files"] != expected:
    ...
    if len(manifest) != 78 or file_count != 91:
        raise ValueError(f"Checkpoint contract changed: {len(manifest)} candidate rows and {file_count} files")
    return {"status": "pass", "mode": "reference" if reference else "learner", "candidate_manifest_rows": len(manifest), "candidate_manifest_bytes": manifest_path.stat().st_size, "candidate_manifest_sha256": sha256(manifest_path), "assembled_files": file_count}


def self_check() -> None:
    with tempfile.TemporaryDirectory(prefix="app1-cp01-") as temp_dir:
        base = Path(temp_dir)
        ...
        assert one == two
        assert one["assembled_files"] == starter["assembled_files"] == 91
        assert one["candidate_manifest_sha256"] == starter["candidate_manifest_sha256"]
        assert "REPLACE" not in (first / "survival-readiness-review.md").read_text(encoding="utf-8")
        assert "REPLACE" in (learner / "survival-readiness-review.md").read_text(encoding="utf-8")
    ...
    print("APP-1 Checkpoint 1 builder self-check passed: 78 accepted component files and 91 assembled files.")
