"""Assemble the APP-2 cumulative Week 3 checkpoint."""
...
MODULES = (
    ("oclc-app2-01", "0.1.0", "module-01", COURSE_ROOT / "modules/01-patient-experience-decision", 25),
    ("oclc-app2-02", "0.1.0", "module-02", COURSE_ROOT / "modules/02-patient-reported-measurement", 66),
    ("oclc-app2-03", "0.1.0", "module-03", COURSE_ROOT / "modules/03-response-representation-bias", 44),
)
...
def assemble(target: Path, reference: bool = False) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="app2-cp01-modules-") as temp_dir:
        temp = Path(temp_dir)
    ...
    if len(manifest) != 135 or file_count != 149:
...
def self_check() -> None:
    with tempfile.TemporaryDirectory(prefix="app2-cp01-") as temp_dir:
        base = Path(temp_dir)
        ...
        assert one == two
        assert one["candidate_manifest_rows"] == 135 and one["assembled_files"] == starter["assembled_files"] == 149
        assert one["candidate_manifest_sha256"] == starter["candidate_manifest_sha256"]
        assert "REPLACE" not in (first / "measurement-representation-review.md").read_text(encoding="utf-8")
        assert "REPLACE" in (learner / "measurement-representation-review.md").read_text(encoding="utf-8")
    ...
    print("APP-2 Checkpoint 1 builder self-check passed: 135 accepted component files and 149 assembled files.")
