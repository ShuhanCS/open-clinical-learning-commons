"""Freeze an accepted APP-3 Module 07 candidate for final review."""
...
CP1_RELEASE = COURSE_ROOT / "checkpoints/01-measures-variation-readiness/release.json"
CP2_RELEASE = COURSE_ROOT / "checkpoints/02-forecast-scenario-monitoring-release/release.json"
...
def candidate_inventory(candidate: Path) -> list[dict[str, object]]:
    ...
    if fields != MODULE_MANIFEST_FIELDS or len(immutable) != 389:
    ...
    if len(paths) != 416 or len(set(paths)) != 416:
        raise ValueError("Module 07 candidate must contain exactly 416 unique files")
    return [{
...
def assemble(candidate: Path, target: Path, reference: bool = False) -> dict[str, object]:
    ...
    if actual != expected or len(actual) != 431:
        raise ValueError("Final checkpoint tree must contain exactly 431 expected files")
    return {
...
def self_check() -> None:
    with tempfile.TemporaryDirectory(prefix="app3-final-assemble-") as temp_dir:
        base = Path(temp_dir)
        ...
        assert one == two
        assert one["candidate_files"] == 416 and one["assembled_files"] == 431
    ...
    print("APP-3 final assembler self-check passed: 416 frozen candidate files and 431 total files.")
...
def main() -> None:
    ...
    try:
        if args.reference:
            ...
            with tempfile.TemporaryDirectory(prefix="app3-final-reference-") as temp_dir:
