"""Assemble the APP-3 cumulative Week 6 checkpoint."""
...
MODULES = (
    ("oclc-app3-04", "0.1.0", "module-04", COURSE_ROOT / "modules/04-demand-forecasting-capacity", 59),
    ("oclc-app3-05", "0.1.0", "module-05", COURSE_ROOT / "modules/05-improvement-scenarios-evaluation", 68),
    ("oclc-app3-06", "0.1.0", "module-06", COURSE_ROOT / "modules/06-feasibility-monitoring-embedded-ml", 82),
)
IMMUTABLE_FILES = (
...
def assemble(target: Path, reference: bool = False) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="app3-cp02-modules-") as temp_dir:
        temp = Path(temp_dir)
    ...
    if len(manifest) != 209 or file_count != 226:
...
def self_check() -> None:
    with tempfile.TemporaryDirectory(prefix="app3-cp02-") as temp_dir:
        base = Path(temp_dir)
        ...
        assert one == two
        assert one["candidate_manifest_rows"] == 209
        assert one["assembled_files"] == starter["assembled_files"] == 226
        assert one["candidate_manifest_sha256"] == starter["candidate_manifest_sha256"]
        assert "REPLACE" not in (first / "forecast-scenario-monitoring-review.md").read_text(encoding="utf-8")
        assert "REPLACE" in (learner / "forecast-scenario-monitoring-review.md").read_text(encoding="utf-8")
    ...
    print("APP-3 Checkpoint 2 builder self-check passed: 209 accepted component files and 226 assembled files.")
