...
The seventh runnable unit is [APP-3 Module 07: Clinician leadership, recommendation, and defense](courses/clinical-performance-improvement/modules/07-clinician-leadership-defense/README.md), governed by the durable [Module 07 specification](docs/curriculum/courses/APP-3/modules/07-clinician-leadership-defense-spec.md). It freezes both accepted checkpoints in a 416-file leadership candidate, preserves every failed or unavailable result, assigns decision and disagreement ownership, and records `accept with conditions` separately from the clinical recommendation `revise before testing`. Joe Joseph, MD, SFHM, is the designated clinician of record under a dated identity boundary; participation details remain a pre-alpha condition.

The [APP-3 final checkpoint](courses/clinical-performance-improvement/checkpoints/03-clinical-performance-improvement-package/README.md) freezes all 416 Module 07 files and adds the reviewed 35-point score, 26 final gates, 14-question defense, 13 reviewer roles, reproduction, audit, 12 conditions, package disposition, and separate clinical performance recommendation. Its durable [checkpoint specification](docs/curriculum/courses/APP-3/checkpoints/03-clinical-performance-improvement-package-spec.md) records `accept with conditions` for the curriculum package and `revise before testing` for the clinical proposal. It closes APP-3 for curriculum construction without authorizing clinical action, staffing or schedule change, automation, testing, implementation, production scoring, or model deployment.
...
The sixth runnable unit is [APP-5 Module 06: Accountable intervention design, monitoring, and embedded ML](courses/population-health-equity/modules/06-intervention-monitoring-embedded-ml/README.md), governed by its durable [Module 06 specification](docs/curriculum/courses/APP-5/modules/06-intervention-monitoring-embedded-ml-spec.md). It freezes the complete 340-file Module 05 reference, reconciles 280 fictional monitoring tests, reports all 20 measures and six human-review triggers, and preserves feedback, recourse, incident, pause, stop, revision, and retirement. Its fixed four-cluster challenger passes alternate-seed checks but fails the preregistered scaling and selected-area coverage standards, so the model is rejected without tuning and the transparent community-review comparison remains unchanged. All 34 gates pass, permitting construction of the separate Week 6 checkpoint while real intervention use and Module 07 remain prohibited.
...
The [APP-5 final checkpoint](courses/population-health-equity/checkpoints/03-population-intervention-analytics-plan/README.md), governed by its durable [checkpoint specification](docs/curriculum/courses/APP-5/checkpoints/03-population-intervention-analytics-plan-spec.md), freezes all 1,328 Module 07 files in a 1,343-file final package. It records the 35-point component once, requires all 26 gates, preserves all 26 inherited conditions, 18 reviewer roles, and the 16-question defense, and closes APP-5 for curriculum construction as `accept with conditions`. The separate fictional planning recommendation remains `recommend seeking approval for bounded structured community review`; it creates no contact, allocation, service, effect, implementation, or deployment authority.
...
1. Specify APP-6 and build Module 01, From a question to a causal estimand and study design.
2. Continue APP-6 and APP-7 one distinct module and checkpoint at a time.
